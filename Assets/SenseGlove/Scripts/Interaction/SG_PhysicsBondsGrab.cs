...
namespace SG
{
    public class SG_PhysicsBondsGrab : SG_GrabScript
    {
        private BondForceFeedback bff;
        ...
        public override void UpdateGrabLogic(float dT)
        {
            ...
            if (this.IsGrabbing) //Check for release - Gesture Based
            {
                ...
                if(this.heldObjects[0].CompareTag("bond")){
                    BondForceFeedback bff = new BondForceFeedback();
                    this.heldObjects[0].GetComponent<MeshRenderer>().enabled = false;
                    this.heldObjects[0].GetComponent<BondInformation>().isGrabbed = true;
                    bff.OnGrabTriggered(heldObjects[0].name, 0.1f);

                    //this.heldObjects[0].defaultMaterial.color = newColor;
                }
                this.heldObjects[0].GetComponent<MeshRenderer>().enabled = true;
                EvaluateRelease();
            }
